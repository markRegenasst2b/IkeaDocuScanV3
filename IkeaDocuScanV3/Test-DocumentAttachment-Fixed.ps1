...
# Force all TLS/SSL protocols - MUST be done before any web requests
...
Write-Host "Security Protocol: $([System.Net.ServicePointManager]::SecurityProtocol)" -ForegroundColor Gray
Write-Host ""
...
try {
    ...
    # Use the method that works (from Test 6)
    $diagnostic = Invoke-RestMethod -Uri $testUrl -Method GET -UseDefaultCredentials

    # Check if we got data
    if (-not $diagnostic) {
        Write-Host "[ERROR] No data returned from diagnostic endpoint" -ForegroundColor Red
        exit 1
    }

    # Display timestamp
    Write-Host "Diagnostic run at: $($diagnostic.timestamp)" -ForegroundColor Gray
    Write-Host ""

    # Display each check
    if ($diagnostic.checks) {
        foreach ($check in $diagnostic.checks) {
            ...
            switch ($check.status) {
                "success" {
                    ...
                    if ($null -ne $check.templates) {
                        foreach ($template in $check.templates) {
                            ...
                            Write-Host "      Hex Bytes: $($template.templateKeyBytes)" -ForegroundColor Gray
                        }
                    }
                ...
                "error" {
                    ...
                    if ($check.stackTrace) {
                        Write-Host "    Stack trace available (run with -Verbose to see)" -ForegroundColor Gray
                    }
                }
            }
    ...
    if ($diagnostic.summary) {
        ...
        Write-Host "[WARNING] No summary data in response" -ForegroundColor Yellow
        Write-Host "Response structure:" -ForegroundColor Gray
        $diagnostic | ConvertTo-Json -Depth 3 | Write-Host -ForegroundColor Gray
    }
