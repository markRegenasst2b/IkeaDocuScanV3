...
<#
.SYNOPSIS
    Publishes the IkeaDocuScan Web application in Debug mode with automatic version incrementing.

.DESCRIPTION
    This script performs the following operations:
    1. Checks for pending git changes (stops if any exist)
    2. Deletes the existing publish folder (d:\pub)
    3. Increments the version number in the .csproj file
    4. Adds current date to VersionSuffix (e.g., testdeploy1119)
    5. Cleans and publishes the application in Debug mode
    6. Creates a zip file of the published content
